################################################################################
# Automatically-generated file. Do not edit!
################################################################################

# Add inputs and outputs from these tool invocations to the build variables 
C_SRCS += \
../settings/backlight.c \
../settings/connection.c \
../settings/outputs.c \
../settings/settings.c \
../settings/time-settings.c 

OBJS += \
./settings/backlight.o \
./settings/connection.o \
./settings/outputs.o \
./settings/settings.o \
./settings/time-settings.o 

C_DEPS += \
./settings/backlight.d \
./settings/connection.d \
./settings/outputs.d \
./settings/settings.d \
./settings/time-settings.d 


# Each subdirectory must supply rules for building sources it contributes
settings/%.o: ../settings/%.c
	@echo 'Building file: $<'
	@echo 'Invoking: AVR Compiler'
	avr-gcc -Wall -Os -fpack-struct -fshort-enums -ffunction-sections -fdata-sections -std=gnu99 -funsigned-char -funsigned-bitfields -mmcu=atmega32 -DF_CPU=16000000UL -MMD -MP -MF"$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -c -o "$@" "$<"
	@echo 'Finished building: $<'
	@echo ' '


